...
run_metatest<-function(mat){
  tryCatch({  meta_obj <- metatest(formula = y~factor(C_1)+R1, variance = variance, data=mat)
...
df_res <- ddply(data_mat, .(Isoform), .fun = failwith(NULL,run_metatest), .parallel = T, .paropts = list(.packages='metatest'))
stopCluster(cl)
